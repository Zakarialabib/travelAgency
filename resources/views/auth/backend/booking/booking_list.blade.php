                        ...
                        <thead>
                            <thead>
                                ...
                                <div class="btn-group row">
                                    @if(isset($booking['place']))
                                        <button type="button" data-target="modal_booking_detail"  class="btn-sm btn-primary booking_detail"
                                                data-id="{{$booking->id}}"
                                        ...
                                        @if($booking->status === \App\Booking::STATUS_PENDING || $booking->status === \App\Booking::STATUS_DEACTIVE)
                                            <form class="d-inline" action="{{route('booking_update_status')}}" method="POST">
                                                ...
                                                <button type="button" class="btn-sm btn-success booking_approve" data-id="{{$booking->id}}">{{__('Approve')}}</button>
                                            </form>
                                        @endif
                                        @if($booking->status === \App\Booking::STATUS_PENDING || $booking->status === \App\Booking::STATUS_ACTIVE)
                                            <form class="d-inline" action="{{route('booking_update_status')}}" method="POST">
                                                ...
                                                <button type="button" class="btn-sm btn-danger booking_cancel">{{__('Cancel')}}</button>
                                            </form>
                                        @endif
                                    @else
                                        <i>{{__('Place deleted')}}</i>
                                    @endif
                                    <a class="btn-sm btn-warning" href="{{route('booking_edit', $booking->id)}}">{{__('Edit')}}</a>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
