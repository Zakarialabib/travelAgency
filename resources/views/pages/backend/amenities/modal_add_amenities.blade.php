<div class="modal fade" id="modal_add_amenities" tabindex="-1" role="dialog" aria-labelledby="modal_add_amenities" aria-hidden="true">
    <div class="modal-dialog modal-lg" style="padding: 60px;" style="padding: 60px;" role="document">
        <div class="modal-content">
            ...
            <form action="{{route('amenities_create')}}" method="post" enctype="multipart/form-data" data-parsley-validate>
                                ...
                                @foreach($languages as $index => $language)
                                    <div class="tab-pane fade show {{$index !== 0 ?: "active"}}" id="language_{{$language->code}}" role="tabpanel" aria-labelledby="home-tab">
                                        <div class="form-group">
                                            <label for="name">{{__('Amenities Name')}} <small>({{$language->code}})</small>: *</label>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-primary" id="submit_add_amenities">{{__('Add')}}</button>
                    <button class="btn btn-primary" id="submit_edit_amenities">{{__('Save')}}</button>
