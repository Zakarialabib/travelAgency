                ...
                <div class="field-select">
                    <select class="map_filter" id="category_id">
                        ...
                        @foreach ($categories as $cat)
                            <option value="{{ $cat->id }}" {{ isSelected($cat->slug, $cat_slug) }}>{{ $cat->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="container">
                    <div class="city-content__tabtitle__tablist">
                        <ul style="padding-inline-start: inherit;">
                            <li class="{{ 'city_detail' }}"><a href="{{ route('city_detail', $city->slug) }}"
                                    title="{{ $city->name }}">{{ $city->name }}</a></li>
                            @foreach ($categories as $cat)
                                <li class="{{ isActive($cat->slug, $cat_slug) }}"><a
                                        href="{{ route('city_category_detail', [$city->slug, $cat->slug]) }}"
                                        title="{{ $cat->name }}">{{ $cat->name }}</a></li>
                            @endforeach
                        </ul>
                    </div><!-- .city-content__tabtitle__tablist -->
                    <a class="city-content__tabtitle__button btn btn-mapsview" title="{{ __('Maps view') }}"
                        href="?view=map">
                            ...
                            @foreach ($features as $feature)
                                <div class="city-content__item">
                                    <h2 class="title title--more">
                                        ...
                                        <a title="{{ __('See all') }}"
                                            href="{{ route('city_category_detail', [$city->slug, $feature['category_slug']]) }}">{{ __('See all') }}
                                            ({{ count($feature['places']) }})</a>
                                    </h2>
                    ...
                    @else
                        <div class="city-content__panel">
                            <div class="shop__meta">
                                <h2 class="title title--result">{{ $places_by_category['category']['name'] }}
                                    <span>({{ $places_by_category['places']->total() }} {{ __('results') }})</span></h2>
                            ...
                            <div class="golo-menu-filter">
                                ...
                                <input type="hidden" name="category_id"
                                    value="{{ $places_by_category['category']['id'] }}">
                            </div>
